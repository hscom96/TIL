...
- [아이템 51. 메서드 시그니처를 신중히 설계하라](#아이템-51-메서드-시그니처를-신중히-설계하라)
  ...
  - [매개변수 목록은 짧게 유지하자](#매개변수-목록은-짧게-유지하자)
    - [짧게 줄이는 3가지 기술](#짧게-줄이는-3가지-기술)
      - [1. 여러 메서드로 쪼갠다](#1-여러-메서드로-쪼갠다)
      - [2. 매개변수 여러 개를 묶어주는 도우미 클래스를 만든다.](#2-매개변수-여러-개를-묶어주는-도우미-클래스를-만든다)
      - [3. 객체 생성에 사용한 빌더패턴을 메서드 호출에 응용](#3-객체-생성에-사용한-빌더패턴을-메서드-호출에-응용)
  - [매개변수 타입으로는 클래스보다는 인터페이스가 더 낫다](#매개변수-타입으로는-클래스보다는-인터페이스가-더-낫다)
  - [boolean 보다는 원소 2개짜리 열거 타입이 낫다](#boolean-보다는-원소-2개짜리-열거-타입이-낫다)
  - [Reference](#reference)
