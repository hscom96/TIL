...
- [아이템 47. 반환 타입으로는 스트림보다는 컬렉션이 낫다](#아이템-47-반환-타입으로는-스트림보다는-컬렉션이-낫다)
  - [API 원소 sequence 반환 타입](#api-원소-sequence-반환-타입)
  - [스트림 반환 문제해결 방법](#스트림-반환-문제해결-방법)
    - [`Stream<E>`를 `Iterable<E>`로 중개해주는 어댑터 사용](#streame를-iterablee로-중개해주는-어댑터-사용)
      - [사용 예시](#사용-예시)
  - [`Iterable` 반환 문제해결 방법](#iterable-반환-문제해결-방법)
    - [`Iterable<E>`를 `Stream<E>`로 중개해주는 어댑터 사용](#iterablee를-streame로-중개해주는-어댑터-사용)
  - [공개 API 작성 할때는 스트림 파이프라인 or 반복문 처리 원하는 사용자 모두 고려하자](#공개-api-작성-할때는-스트림-파이프라인-or-반복문-처리-원하는-사용자-모두-고려하자)
    - [전용 컬렉션 예시](#전용-컬렉션-예시)
      - [모든 부분 리스트를 스트림으로 반환하는 방식](#모든-부분-리스트를-스트림으로-반환하는-방식)
