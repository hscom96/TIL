...
- [아이템 28. 배열보다는 리스트를 사용하라](#아이템-28-배열보다는-리스트를-사용하라)
  - [배열 vs 제네릭 차이](#배열-vs-제네릭-차이)
    - [1. 공변 vs 불공변 (invariant)](#1-공변-vs-불공변-invariant)
      - [문제점 예시](#문제점-예시)
    - [2. 배열은 실체화(reify) 된다.](#2-배열은-실체화reify-된다)
  - [제네릭 배열 만들지 못하는 이유는?](#제네릭-배열-만들지-못하는-이유는)
  - [배열 `T[]`보다는 컬렉션 `List<T>`를 사용](#배열-t보다는-컬렉션-listt를-사용)
      - [`T[]` 사용 ver](#t-사용-ver)
      - [`List<T>` 사용 ver](#listt-사용-ver)
  - [결론](#결론)
  - [읽어보면 좋은 Refrence](#읽어보면-좋은-refrence)
