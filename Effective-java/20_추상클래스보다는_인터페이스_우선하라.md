...
- [아이템 20. 추상 클래스보다는 인터페이스를 우선하라.](#아이템-20-추상-클래스보다는-인터페이스를-우선하라)
  - [차이](#차이)
  - [인터페이스 장점](#인터페이스-장점)
      - [기존 클래스에 손쉽게 새로운 인터페이스를 구현해넣을 수 있다.](#기존-클래스에-손쉽게-새로운-인터페이스를-구현해넣을-수-있다)
      - [믹스인(mixin) 정의에 좋다.](#믹스인mixin-정의에-좋다)
      - [계층구조가 없는 타입 프레임워크를 만들 수 있다.](#계층구조가-없는-타입-프레임워크를-만들-수-있다)
      - [래퍼클래스(아이템18)와 함께 사용하면 인터페이스는 기능을 추가할 수 있는 안전한 수단](#래퍼클래스아이템18와-함께-사용하면-인터페이스는-기능을-추가할-수-있는-안전한-수단)
  - [인터페이스 제약](#인터페이스-제약)
      - [디폴트 메서드로 Object 메서드 equals, hashCode 제공하면 안된다.](#디폴트-메서드로-object-메서드-equals-hashcode-제공하면-안된다)
      - [인스턴스 필드를 가질 수 없다.](#인스턴스-필드를-가질-수-없다)
      - [public이 아닌 정적 멤버를 가질 수 없다. (private 정적 메서드는 예외)](#public이-아닌-정적-멤버를-가질-수-없다-private-정적-메서드는-예외)
      - [자신이 만들지 않은 인터페이스에는 디폴트메서드 추가 못한다.](#자신이-만들지-않은-인터페이스에는-디폴트메서드-추가-못한다)
  - [템플릿 메서드 패턴](#템플릿-메서드-패턴)
      - [네이밍 컨벤션](#네이밍-컨벤션)
      - [골격 클래스 적용 예시](#골격-클래스-적용-예시)
  - [시뮬레이트한 다중 상속](#시뮬레이트한-다중-상속)
      - [골격 구현 작성 방법](#골격-구현-작성-방법)
