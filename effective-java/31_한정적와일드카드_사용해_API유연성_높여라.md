...
- [아이템 31. 한정적 와일드카드를 사용해 API 유연성을 높이라](#아이템-31-한정적-와일드카드를-사용해-api-유연성을-높이라)
  - [`<? extends E>` (한정적 와일드카드타입)](#-extends-e-한정적-와일드카드타입)
      - [사용 X (오류발생)](#사용-x-오류발생)
      - [사용 O](#사용-o)
  - [`<? super E>` (한정적 와일드카드타입)](#-super-e-한정적-와일드카드타입)
      - [사용 X (오류발생)](#사용-x-오류발생-1)
      - [사용 O](#사용-o-1)
  - [주의사항](#주의사항)
      - [유연성을 극대화하려면?](#유연성을-극대화하려면)
      - [반면, 입력 매개변수가 생산자와 소비자 역할을 동시에 한다면?](#반면-입력-매개변수가-생산자와-소비자-역할을-동시에-한다면)
      - [와일드 카드 사용 구분](#와일드-카드-사용-구분)
      - [반환 타입에는 한정적 와일드카드 타입을 쓰면 안된다.](#반환-타입에는-한정적-와일드카드-타입을-쓰면-안된다)
      - [클래스 사용자가 와일드 타입을 쓰였는지 모르게 신경 안쓰게하자.](#클래스-사용자가-와일드-타입을-쓰였는지-모르게-신경-안쓰게하자)
      - [Comparable은 언제나 소비자이므로 일반적으로 `Comparable<E>`보다는 `Comparable<? super E>`를 사용하는게 좋다.](#comparable은-언제나-소비자이므로-일반적으로-comparablee보다는-comparable-super-e를-사용하는게-좋다)
  - [메서드 선언에 타입 매개변수가 한 번만 나오면 와일드 카드로 대체하라.](#메서드-선언에-타입-매개변수가-한-번만-나오면-와일드-카드로-대체하라)
      - [private 도우미 메서드](#private-도우미-메서드)
  - [Reference](#reference)
