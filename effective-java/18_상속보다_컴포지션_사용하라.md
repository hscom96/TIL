...
- [아이템 18. 상속보다는 컴포지션을 사용하라.](#아이템-18-상속보다는-컴포지션을-사용하라)
  - [상속의 위험성](#상속의-위험성)
    - [하위 클래스가 깨지기 쉬운 이유](#하위-클래스가-깨지기-쉬운-이유)
      - [1. 상위 클래스 내부 구현을 알 수가 없어 재정의시 원하지 않는 동작이 발생할 수 있다. (HashSet 예제의 핵심)](#1-상위-클래스-내부-구현을-알-수가-없어-재정의시-원하지-않는-동작이-발생할-수-있다-hashset-예제의-핵심)
      - [2. 상위 클레스에 새로운 메서드가 추가되었을때 위험할 수 있다.](#2-상위-클레스에-새로운-메서드가-추가되었을때-위험할-수-있다)
      - [3. 새로 추가된 상위 클레스 메소드가 하위 클래스에 이미 추가되었던 메서드와 시그니처가 같고 반환 타입이 다르다면?](#3-새로-추가된-상위-클레스-메소드가-하위-클래스에-이미-추가되었던-메서드와-시그니처가-같고-반환-타입이-다르다면)
  - [컴포지션](#컴포지션)
      - [용어 정리](#용어-정리)
      - [효과](#효과)
      - [구현방법](#구현방법)
      - [래퍼 클래스 단점](#래퍼-클래스-단점)
      - [컴포지션 사용할 상황에 상속 사용하면?](#컴포지션-사용할-상황에-상속-사용하면)
  - [결론](#결론)
  - [용어정리](#용어정리)
