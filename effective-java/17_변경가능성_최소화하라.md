...
- [아이템 17. 변경 가능성을 최소화하라](#아이템-17-변경-가능성을-최소화하라)
  - [불변 클래스란?](#불변-클래스란)
  - [불변 클래스 생성 5가지 규칙](#불변-클래스-생성-5가지-규칙)
      - [1. 객체의 상태를 변경하는 메서드를 제공하지 않는다.](#1-객체의-상태를-변경하는-메서드를-제공하지-않는다)
  ...
  - [불변 클래스 예시](#불변-클래스-예시)
    - [예시 주목할 부분](#예시-주목할-부분)
  - [불변 클래스 장점](#불변-클래스-장점)
      - [불변 객체는 Thread safe하여 따로 동기화할 필요 없다.](#불변-객체는-thread-safe하여-따로-동기화할-필요-없다)
      - [불변 객체는 인스턴스를 재활용하는 것을 권장된다.](#불변-객체는-인스턴스를-재활용하는-것을-권장된다)
      - [불변 객체는 방어적복사가 필요 없다.](#불변-객체는-방어적복사가-필요-없다)
      - [불변객체는 자유롭게 공유할 수 있고, 불변 객체끼리 내부 데이터를 공유할 수 있다.](#불변객체는-자유롭게-공유할-수-있고-불변-객체끼리-내부-데이터를-공유할-수-있다)
        - [객체를 만들 때 다른 불변 객체들을 구성요소로 사용하면 이점이 많다.](#객체를-만들-때-다른-불변-객체들을-구성요소로-사용하면-이점이-많다)
