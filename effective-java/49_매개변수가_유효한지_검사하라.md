...
- [아이템 49. 매개변수가 유효한지 검사하라](#아이템-49-매개변수가-유효한지-검사하라)
  - [메서드, 생성자에서 매개변수가 특정 조건을 만족해야한다면?](#메서드-생성자에서-매개변수가-특정-조건을-만족해야한다면)
      - [문서화하자!](#문서화하자)
      - [몸체 시작되기 전에 검사해야한다!](#몸체-시작되기-전에-검사해야한다)
  - [null 검사 방법](#null-검사-방법)
  - [public 아닌 메서드라면?](#public-아닌-메서드라면)
      - [`assert`](#assert)
  - [주의사항](#주의사항)
      - [메서드가 직접 사용하지는 않으나 나중에 쓰기 위해 저장하는 매개변수는 특히 신경쓰자 (디버깅 괴로워진다..)](#메서드가-직접-사용하지는-않으나-나중에-쓰기-위해-저장하는-매개변수는-특히-신경쓰자-디버깅-괴로워진다)
      - [몸체 실행전 매개변수 유효성 검사해야한다는 규칙 예외가 있다](#몸체-실행전-매개변수-유효성-검사해야한다는-규칙-예외가-있다)
      - [이번 아이템 "매개변수에 제약 두는 게 좋다"라고 해석하면 안된다.](#이번-아이템-매개변수에-제약-두는-게-좋다라고-해석하면-안된다)
