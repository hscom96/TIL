...
- [아이템 86. Serializable을 구현할지는 신중히 결정하라](#아이템-86-serializable을-구현할지는-신중히-결정하라)
  - [`Serializable` 이란?](#serializable-이란)
  - [첫 번째 문제 :  `Serializable`을 구현하면 릴리스한 뒤에는 수정하기 어렵다.](#첫-번째-문제---serializable을-구현하면-릴리스한-뒤에는-수정하기-어렵다)
    - [직렬화가 클래스 개선 방해하는 사례](#직렬화가-클래스-개선-방해하는-사례)
  - [두 번째 문제 : `Serializable`구현은 버그와 보안 구멍이 생길 위험이 높아진다.](#두-번째-문제--serializable구현은-버그와-보안-구멍이-생길-위험이-높아진다)
    - [예시](#예시)
  - [Serializable 구현 여부는 가볍게 결정할 사안이 아니다.](#serializable-구현-여부는-가볍게-결정할-사안이-아니다)
  - [세 번째 문제 : `Serializable` 구현은 해당 클래스의 신버전을 릴리스할 때 테스트할 것이 늘어난다.](#세-번째-문제--serializable-구현은-해당-클래스의-신버전을-릴리스할-때-테스트할-것이-늘어난다)
  - [상속용 클래스, 인터페이스의 경우](#상속용-클래스-인터페이스의-경우)
  - [주의사항 (클래스가 직렬화, 확장 가능하면)](#주의사항-클래스가-직렬화-확장-가능하면)
  - [내부 클래스는 직렬화를 구현하면 안된다.](#내부-클래스는-직렬화를-구현하면-안된다)
  - [참조](#참조)
