...
- [아이템 13. clone 재정의는 주의해서 진행하라](#아이템-13-clone-재정의는-주의해서-진행하라)
  - [시작하기 앞서 ..](#시작하기-앞서-)
    - [복제는 왜 하는거야?](#복제는-왜-하는거야)
    - [얕은 복제?](#얕은-복제)
    - [깊은 복제?](#깊은-복제)
  - [Cloneable 특이함](#cloneable-특이함)
  - [Cloneable 일반 규약](#cloneable-일반-규약)
      - [1) x.clone() != x는 참이다.](#1-xclone--x는-참이다)
      - [2) x.clone().getClass() == x.getClass()는 참이지만 반드시 만족해야 하는 것은 아니다.](#2-xclonegetclass--xgetclass는-참이지만-반드시-만족해야-하는-것은-아니다)
      - [3) x.clone().equals(x)는 참이지만 필수는 아니다.](#3-xcloneequalsx는-참이지만-필수는-아니다)
      - [4) x.clone().getClass() == x.getClass()](#4-xclonegetclass--xgetclass)
    - [Cloneable 주의할점](#cloneable-주의할점)
  - [Cloneable 적용](#cloneable-적용)
    - [1) 불변 객체경우 (가변 상태 참조 x)](#1-불변-객체경우-가변-상태-참조-x)
    - [2) 객체가 가변 객체를 참조하는 경우](#2-객체가-가변-객체를-참조하는-경우)
      - [해결법 #1](#해결법-1)
  - [복사 생성자와 복사 팩터리 (추천방법)](#복사-생성자와-복사-팩터리-추천방법)
      - [복사 생성자](#복사-생성자)
      - [복사 팩터리](#복사-팩터리)
    - [장점](#장점)
  - [용어 정리](#용어-정리)
