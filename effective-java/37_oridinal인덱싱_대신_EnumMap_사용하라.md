...
- [아이템 37. ordinal 인덱싱 대신 EnumMap을 사용하라](#아이템-37-ordinal-인덱싱-대신-enummap을-사용하라)
  - [(따라하지말자) ordinal() 배열 인덱스 사용 #1](#따라하지말자-ordinal-배열-인덱스-사용-1)
    - [문제점](#문제점)
      - [1. 배열은 제네릭과 호환되지 않음 => 비검사 형변환](#1-배열은-제네릭과-호환되지-않음--비검사-형변환)
      - [2. 배열은 각 인덱스의 의미를 모르니 출력 결과에 직접 레이블을 달아야함](#2-배열은-각-인덱스의-의미를-모르니-출력-결과에-직접-레이블을-달아야함)
      - [3. 정수는 열거 타입과 달리 타입안전하지 않다.](#3-정수는-열거-타입과-달리-타입안전하지-않다)
  - [(개선) EnumMap](#개선-enummap)
    - [장점](#장점)
  - [스트림](#스트림)
    - [EnumMap 사용 x](#enummap-사용-x)
      - [단점](#단점)
    - [EnumMap 사용](#enummap-사용)
  - [(따라하지말자) ordinal() 배열 인덱스 사용 #2](#따라하지말자-ordinal-배열-인덱스-사용-2)
    - [단점](#단점-1)
  - [EnumMap](#enummap)
