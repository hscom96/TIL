...
- [아이템 52. 오버로딩는 신중히 사용하라](#아이템-52-오버로딩는-신중히-사용하라)
  - [오버로딩](#오버로딩)
      - [해결책은?](#해결책은)
  - [overriding(재정의)](#overriding재정의)
  - [주의사항](#주의사항)
      - [매개변수 수가 같은 오버로딩은 웬만하면 만들지 말자.](#매개변수-수가-같은-오버로딩은-웬만하면-만들지-말자)
      - [가변 인수를 사용하는 메서드는 오버로딩를 사용하지 말자.](#가변-인수를-사용하는-메서드는-오버로딩를-사용하지-말자)
      - [오버로딩대신 메서드 이름을 다르게 지어줄 수 있다.](#오버로딩대신-메서드-이름을-다르게-지어줄-수-있다)
      - [매개변수 개수가 같은 오버로딩을 피할 수 없다면?](#매개변수-개수가-같은-오버로딩을-피할-수-없다면)
      - [메서드를 다중정의할때, 서로 다른 함수형 인터페이스라도 같은 위치의 인수로 받아서는 안된다.](#메서드를-다중정의할때-서로-다른-함수형-인터페이스라도-같은-위치의-인수로-받아서는-안된다)
  - [결론](#결론)
